...
export async function getTTSHistory(client, options = {}) {
    const { limit, offset = 0, status, tts_type, language, date_from, date_to, device_id, } = options;
    if (limit !== undefined && (!Number.isInteger(limit) || limit < 1)) {
        throw new HasabValidationError("Limit must be an integer greater than or equal to 1.");
    }
    ...
    if (tts_type && !["regular", "reference"].includes(tts_type)) {
        throw new HasabValidationError("tts_type must be 'regular' or 'reference'.");
    }
    if (language && (typeof language !== "string" || language.length > 10)) {
        throw new HasabValidationError("Language must be a string (max 10 characters).");
    }
    if (date_from && typeof date_from !== "string") {
        throw new HasabValidationError("date_from must be a valid ISO date string.");
    }
