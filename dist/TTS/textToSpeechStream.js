import { HasabApiError, HasabNetworkError, HasabUnknownError, HasabValidationError, } from "../common/errors.js";
import axios from "axios";
export async function ttsStream(request, client, onData, onError, onComplete) {
    let cancelled = false;
    const { text, language, speaker_name, sample_rate } = request;
    if (!text || typeof text !== "string" || text.trim().length === 0) {
        onError(new HasabValidationError("Text is required and must not be empty."));
        return () => { };
    }
    ...
    try {
        ...
        if (axios.isAxiosError(err)) {
            const axiosErr = err;
            error = new HasabApiError("Failed to start TTS stream", axiosErr.response?.status ?? 500, axiosErr.response?.data);
        }
        else {
    ...
    const processChunk = (raw) => {
        ...
        try {
            ...
            onError(new HasabNetworkError(`Decode error: ${decodeErr instanceof Error ? decodeErr.message : "Unknown"}`));
            return true;
        }
    ...
    const onDataHandler = (chunk) => {
        try {
            ...
            onError(err instanceof Error ? err : new HasabUnknownError("Handler error"));
            cleanup();
        }
    };
    const onEndHandler = () => {
        try {
            ...
            onError(err instanceof Error ? err : new HasabUnknownError("End handler error"));
        }
        finally {
            cleanup();
        }
...
//# sourceMappingURL=textToSpeechStream.js.map