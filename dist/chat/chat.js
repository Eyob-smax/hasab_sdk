...
import { HasabError, HasabApiError, HasabNetworkError, HasabValidationError, } from "../common/errors.js";
export async function chat(message, client, options) {
    try {
        ...
        if (axios.isAxiosError(error)) {
            if (error.response) {
                throw new HasabApiError(error.response.data?.message ?? "Chat API error", error.response.status, error.response.data);
            }
            if (error.request) {
