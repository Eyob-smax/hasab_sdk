...
import { HasabValidationError, HasabApiError, HasabNetworkError, HasabAuthError, HasabRateLimitError, HasabTimeoutError, HasabUnknownError, } from "../common/errors.js";
export async function transcribe(request, client) {
    ...
    try {
        const response = await client.post(`/upload-audio`, form, {
            headers: {
            ...
            if (axiosErr.response) {
                ...
                switch (status) {
                    ...
                    case 429:
                        ...
                        throw new HasabRateLimitError("Rate limit exceeded", Number(retryAfter) || undefined);
                    case 500:
                    case 502:
                    case 503:
                    case 504:
        ...
        throw new HasabUnknownError(error instanceof Error ? error.message : "Unknown error");
    }
}
