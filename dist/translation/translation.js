...
export async function translate(text, client, targetLanguage, sourceLanguage) {
    if (!text || typeof text !== "string") {
        throw new HasabValidationError("Text to translate is required and must be a string.");
    }
    if (text.trim().length === 0) {
        throw new HasabValidationError("Text to translate cannot be empty or whitespace.");
    }
    if (!targetLanguage || typeof targetLanguage !== "string") {
        throw new HasabValidationError("Target language is required and must be a string.");
    }
    if (sourceLanguage && typeof sourceLanguage !== "string") {
        throw new HasabValidationError("Source language must be a string if provided.");
    }
    const payload = {
        text: text.trim(),
